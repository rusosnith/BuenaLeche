...
from buenaleche.drive_sync import build_drive_service, download_pdf, folder_id_from_url, list_pdfs
from buenaleche.parser import parse_pdf


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Actualiza dataset y dashboard desde PDFs en Google Drive")
    ...
    for file_item in reversed(files):
        ...
        if file_state and file_state.get("modified_time") == file_item.modified_time:
            ...
            if report_path.exists():
                reports.append(parse_pdf(report_path, source_id=file_item.id))
                continue

        local_path = pdf_dir / file_item.name
        download_pdf(service, file_item.id, local_path)
        report = parse_pdf(local_path, source_id=file_item.id)
